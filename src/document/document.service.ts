...
export class DocumentService {
  ...
  async uploadDocuments(
    ...
    if (jobId) {
      const currentCount = await this.documentRepository.countByJobId(
        projectId,
        jobId
      );
      ...
      const currentCount =
        await this.documentRepository.countByProjectId(projectId);
  ...
  async deleteDocument(documentId: number, projectId: number): Promise<void> {
    ...
    const chunks =
      await this.documentChunkRepository.findByDocumentId(documentId);
    const vectorIds = chunks.map(c => `chunk-${c.documentId}-${c.chunkIndex}`);
    await this.pineconeService.deleteChunksByDocumentId(
      vectorIds,
      doc.projectId
    );
  ...
  async getExtractedTextsForJob(
    ...
    const texts = docs
      .filter(
        doc =>
          doc.extractionStatus === ('done' as ExtractionStatus) &&
  ...
  async getRelevantChunksForJob(
    ...
    if (!this.pineconeService.isDisabled()) {
      const results = await this.pineconeService.queryChunks(
        queryEmbedding,
        projectId,
        jobId,
        topK
      ...
      console.warn(
        '[Pinecone] No results returned — falling back to Postgres cosine similarity'
      );
    }

    // Fallback: Postgres cosine similarity
    const allChunks = await this.documentChunkRepository.findByJobId(
      projectId,
      jobId
    );
