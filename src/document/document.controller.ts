...
export class DocumentController {
  ...
  getJobDocuments = async (
    req: Request<JobDocumentParams>,
    res: Response
  ): Promise<void> => {
    try {
      const projectId = parseInt(req.params.projectId);
      const jobId = parseInt(req.params.jobId);
      const documents = await this.documentService.getDocumentsByJob(
        projectId,
        jobId
      );
      res.json(documents);
  ...
  uploadDocuments = async (
    ...
    try {
      ...
      const documents = await this.documentService.uploadDocuments(
        projectId,
        files
      );
